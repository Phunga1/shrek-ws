...
while rclpy.ok():
    rclpy.spin_once(node)  #laat de node de /odom waarden opnemen
    inc_x = goalx - x   #verschil in x berekenen
    inc_y = goaly - y   #verschil in y berekenen
    print(goalx,goaly)
    angle_to_goal = atan2(inc_y, inc_x) #verschil in hoek berekenen

    if abs(angle_to_goal - theta) > 0.1:   #robot naar het punt laten draaien 
        speed.linear.x=0.0                 #als het verschil in hoek niet klein genoeg is
        speed.angular.z=0.2
    ...
    pub.publish(speed)  # stuur snelheid naar diff_cont/cmd_vel_unstamped
    r.sleep